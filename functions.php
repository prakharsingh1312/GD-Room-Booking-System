...
function show_room_availability($day,$week,$global_times){
	...
	while($room=mysqli_fetch_array($query1)){
		...
		foreach($global_times as $time)
		{
			...
			if(mysqli_num_rows($query)>0)
			{
				$details.='<td><div class="reservation_time_div" style="color:red;"><div class="reservation_time_cell_div">Occupied</div></div></td>';
			}
			else
			{
				$details.='<td><div class="reservation_time_div" style="color:green;"><div class="reservation_time_cell_div">Free</div></div></td>';
			}
		}
		$details.='</tr>';
	}
