...
function delete_user_data($user_id, $data)
{
	...
	else
	{
		...
		elseif($data == 'user')
		{
			...
			mysqli_query($dbconfig,"DELETE FROM " . global_mysqli_group_members_table . " WHERE member_user_id=$user_id")or die('<span class="error_span"><u>mysqli error:</u> ' . htmlspecialchars(mysqli_error($dbconfig)) . '</span>');
		}
...
function show_room_availability($week,$day,$global_times){
	global $dbconfig;
	$query1=mysqli_query($dbconfig,'SELECT * FROM '.global_mysqli_room_details_table);
	$details='';
	while($room=mysqli_fetch_array($query1)){
		$details.='<tr><th class="reservation_time_th">' . $room['room_name'] . '</th>';
		foreach($global_times as $time)
		{
			$query=mysqli_query($dbconfig,"SELECT * FROM ".global_mysqli_reservations_table." WHERE reservation_room_id={$room['room_id']} and reservation_week=$week and reservation_day=$day and reservation_time='$time'");
			//$details.="SELECT * FROM ".global_mysqli_reservations_table." WHERE reservation_room_id={$room['room_id']} and reservation_week=$week and reservation_day=$day and reservation_time='$time'";
			if(mysqli_num_rows($query)>0)
			{
				$details.='<td><div class="reservation_time_div" style="background:red;color:white;pointer-events:none;border:red;"><div class="reservation_time_cell_div"><b>Occupied</b></div></div></td>';
			}
			else
			{
				$details.='<td><div class="reservation_time_div" style="background:green;pointer-events:none;color:white;border:green;"><div class="reservation_time_cell_div"><b>Available</b></div></div></td>';
			}
		}
		$details.='</tr>';
	}
